...
VisibilityPolygon.compute = function (position, segments) {
  ...
  for (var i = 0; i < sorted.length; ) {
    ...
    if (extend) {
      polygon.push(vertex);
      var cur = VisibilityPolygon.intersectLines(
        bounded[heap[0]][0],
        bounded[heap[0]][1],
        position,
        vertex
  ...
  for (var i = 0; i < segments.length; ++i) {
    ...
    for (var j = 0; j < viewport.length; ++j) {
      ...
      if (
        ...
        var intersect = VisibilityPolygon.intersectLines(
          segments[i][0],
          segments[i][1],
          viewport[j],
          viewport[k]
...
VisibilityPolygon.lessThan = function (
  ...
  var inter1 = VisibilityPolygon.intersectLines(
    segments[index1][0],
    segments[index1][1],
    position,
    destination
  );
  var inter2 = VisibilityPolygon.intersectLines(
    segments[index2][0],
    segments[index2][1],
    position,
    destination
